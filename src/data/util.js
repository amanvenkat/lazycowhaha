...
const util = {
    ...
    calcMulti: async function (message) {
        ...
        /** channel */
        if (message.channel.name.toLowerCase().includes('medu-wada')) {
            total += 0.5;
            text += 'Medu Wada as channel\n';
            unlocked += 1;
        }
        ...
        /** username */
        if (guildMemer.user.username.toLowerCase().includes('medu')) {
            total += 2;
            text += 'Username is medu\n';
            unlocked += 1;
        }
        /** nickname */
        if (guildMemer.nickname && guildMemer.nickname.toLowerCase().includes('medu')) {
            total += 2;
            text += 'Nickname is medu\n';
            unlocked += 1;
        }
        /** support server */
        if (message.guild.id === config.support) {
            total += 1;
            text += '[In support server](https://discord.gg/aj3hz97m5U)\n';
            unlocked += 1;
        }
        /** 4:20 */
